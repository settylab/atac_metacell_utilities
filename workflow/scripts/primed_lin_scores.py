...
#        type=list,
#        required=True,
#        help="List of target lineage and celltype",
#    )
#    parser.add_argument(
#        "--max_pval",
#        type=float,
...
def filter_gp_corrs(gp_corrs, gene_list, peak_df, min_corr, max_pval):
    ...
    for gene in list(gp_corrs.keys()):
        if type(gp_corrs[gene]) == int:
            ...
            filtered_gp_corrs[gene] = filtered_gp_corrs[gene][filtered_gp_corrs[gene]['cor']>min_corr]
...
def main():
    ...
    atac_ad = sc.read_h5ad(snakemake.input["atac"])
    rna_ad = sc.read_h5ad(snakemake.input["rna"])
    ...
    target_genes = rna_ad[:, rna_ad.var['upregulated_genes']].obs_names
    control_genes = rna_ad[:, rna_ad.var['control_genes']].obs_names
    print(atac_ad)
    unique_peaks = atac_ad.var.loc[pd.Index(atac_ad.uns[f'{target_lineage}_unique_peaks']), :]

    filtered_gp_corrs = filter_gp_corrs(gene_peak_corrs, target_genes, unique_peaks, min_corr, max_pval)
    control_gp_corrs = filter_gp_corrs(gene_peak_corrs, control_genes, unique_peaks, min_corr, max_pval)
    ...
    for key in ['primed', 'lineage_specific']:
        ...
        for gene in tqdm(filtered_gp_corrs.keys()):
            _concat_imputed_peak_values(atac_ad, imputed_peak_matrix[key], filtered_gp_corrs)
        #get imputed values for peaks associated with control genes
        for gene in tqdm(control_gp_corrs.keys()):
            _concat_imputed_peak_values(atac_ad, imputed_peak_matrix[key], control_gp_corrs)
        #remove duplicates
    ...
    for gene in filtered_gp_corrs.keys():
        index = filtered_gp_corrs[filtered_gp_corrs['category'] == "primed"].index
        atac_ad.var.loc[index, "primed"] = 1
        index = filtered_gp_corrs[filtered_gp_corrs['category'] == "lineage_specifc"].index
        atac_ad.var.loc[index, "lineage_specific"] = 1
    atac_ad.uns['gp_corrs']['target'] = pd.Series(False, index = atac_ad.uns['gp_corrs'].index)
    atac_ad.uns['gp_corrs']['control'] = pd.Series(False, index = atac_ad.uns['gp_corrs'].index)
    for gene in filtered_gp_corrs.keys():
        atac_ad.uns['gp_corrs'].loc[filtered_gp_corrs[gene].index, "target"] = True
    for gene in control_gp_corrs.keys():
        atac_ad.uns['gp_corrs'].loc[control_gp_corrs[gene].index, "control"] = True
    # weighted average
    atac_gene_dfs = dict()
    ...
    for key in imputed_peak_matrix.keys():
        ...
        for gene in filtered_gp_corrs.keys():
            _concat_weighted_peak_avg(atac_gene_dfs[key], imputed_peak_matrix[key], filtered_gp_corrs, gene)
        for gene in control_gp_corrs.keys():
            _concat_weighted_peak_avg(atac_gene_dfs[key], imputed_peak_matrix[key], control_gp_corrs, gene)
        atac_gene_dfs[key] = atac_gene_dfs[key].fillna('0', axis = 1)  
    for key in atac_gene_dfs.keys():
    ...
    print(rna_ad)
        #rna_ad.layers[f'{key}_scores'] = gene_scores.values
    ...
    rna_ad.write_h5ad(snakemake.input["rna"])
    atac_ad.write_h5ad(snakemake.input["atac"])
